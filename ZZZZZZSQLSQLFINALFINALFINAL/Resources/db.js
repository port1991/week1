...
var db = Titanium.Database.open('ListExample');
...
var findAll = function() {
	...
	var row_databse = db.execute('SELECT * FROM ListExample');
	
	console.log("Reading from ListExample");
		
	while (row_databse.validRow) {
		databse_data.push({
			title : row_databse.fieldByName('name'),
			discription : row_databse.fieldByName('description'),
			id : row_databse.fieldByName('id')
			
		});
		
		console.log("Reading person " + row_databse.fieldByName('name'));
	...
	console.log('ListExample contains ' + databse_data.length + ' people.');
	console.log('ListExample contains ' + databse_data);
	
	return databse_data;
};

Ti.API.debug('-Exporting findAll function');
...
exports.remove = function(id) {
	...
	db = Ti.Database.open('ListExample');
	db.execute('DELETE FROM ListExample WHERE id=?', id.id);
	db.close();
	Ti.API.debug('--Open Database DELETE--');
